...
void le_lista(t_lista *a){
    int n;
    printf("\nleia lista, elemento inserido no fim\npara ao digitar zero\n");
    scanf("%d", &n);
    while(n != 0){
        insere_fim_lista(n, a);
        scanf("%d", &n);
    }
}
...
int inicializa_listas (t_lista *l, t_lista *m, t_lista *o, t_lista *c) {
...
int testa_remocoes(t_lista *l, t_lista *o, t_lista *c) {
    ...
    while (! lista_vazia(l)){
        if (! remove_inicio_lista(&item,l)) return 0;
    }
    printf("l: ");
    imprime_lista(l);
    printf("\n");
    ...
    /*while (! lista_vazia(o))*/
    while (o->tamanho)
        if (! remove_fim_lista(&item,o)) return 0;
    printf("o: ");
    imprime_lista(o);
    printf("\n");
...
int main() {
    ...
    if (! inicializa_listas(&l,&m,&o,&c)) return 1; 
