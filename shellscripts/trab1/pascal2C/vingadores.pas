...
var
	herois: ldl; 
	time, bolinho, habilidades, menorconjunto, copia: conjunto;
	acabou: boolean;
	i, j: longint; 
...
begin
	...
	i:=1;
	acabou:=false;
	//ler habilidades dos herois
	while not(acabou) do
	begin
		ler_conjunto(herois[i]);
		acabou:= eh_vazio(herois[i]);
		i:=i+1;
	end;
	//ler habilidades necessarias
	ler_conjunto(habilidades);
	acabou:=false;
	//le times e testa, se eh valido pode ser menor
	//menorconjunto[0]:=MAX;	
	inicializar_conjunto(menorconjunto);
	inicializar_conjunto(time);
	ler_conjunto(time);
	while not eh_vazio(time) do
	begin
		
		for j:=1 to cardinalidade(time) do
		begin
			uniao (herois[time[j]], bolinho, bolinho);
		end;
		//imprimir_conjunto(bolinho);
		if contido(habilidades, bolinho) and (eh_vazio(menorconjunto) 
			or (cardinalidade(time) < cardinalidade(menorconjunto))) then
			begin	
					inicializar_conjunto(menorconjunto);
					copiar_conjunto (time, menorconjunto);
			end;
		//imprimir_conjunto(time);
		//imprimir_conjunto(menorconjunto);
		inicializar_conjunto(bolinho);
		inicializar_conjunto(time);
		ler_conjunto(time);
	end;
	imprimir_conjunto(menorconjunto);
